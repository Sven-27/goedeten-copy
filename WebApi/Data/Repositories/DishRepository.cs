...
namespace Data.Repositories
{
    ...
    public class DishRepository : GenericRepository<Dish>, IDishRepository
    {
        ...
        public async Task<PaginatedList<DishDisplay>> GetFilteredList(int? pageNumber, string sortField,
            string sortOrder,
            int? pageSize, string categoryFilter, string nameFilter, string cookFilter, string cuisineFilter)
        {
            ...
            list = !string.IsNullOrWhiteSpace(categoryFilter)
                ? list.Where(z => z.DishCategory.Name.Contains(categoryFilter))
                : list;
            list = !string.IsNullOrWhiteSpace(nameFilter) ? list.Where(z => z.Name.Contains(nameFilter)) : list;
            list = !string.IsNullOrWhiteSpace(cuisineFilter)
                ? list.Where(z => z.Cuisine.Name.Contains(cuisineFilter))
                : list;
            list = !string.IsNullOrWhiteSpace(cookFilter) ? list.Where(z => z.Cook.Name.Contains(cookFilter)) : list;

            var listNew = from d in list
            ...
            return await PaginatedList<DishDisplay>
                .CreateAsync(listNew.AsNoTracking(), pageNumber ?? 1, pageSize ?? 10, sortField ?? "Id",
                    sortOrder ?? "ASC");
        }
